...
/*
 * Zero-width character filtering for VFS path resolution.
 * Rejects paths containing invisible Unicode characters that can be
 * used to bypass path-based security checks (e.g. CVE-2024-43093).
 *
 * Detected characters (UTF-8 encoded):
 *   U+200B..U+200F  (zero-width space, joiners, direction marks)
 *   U+2028..U+2029  (line/paragraph separator)
 *   U+2060..U+2064  (word joiner, invisible operators)
 *   U+FEFF          (BOM / zero-width no-break space)
 *
 * Returns true if the path contains zero-width characters.
 */
static bool path_has_zero_width_chars(const char *path, int len)
{
	int i;
	...
	for (i = 0; i < len - 2; i++) {
		...
		/* Fast skip: only check bytes starting with 0xE2 or 0xEF */
		if (c == 0xE2 && (i + 2) < len) {
			unsigned char c1 = path[i + 1];
			unsigned char c2 = path[i + 2];
			...
			if (c1 == 0x80 && (c2 == 0xA8 || c2 == 0xA9))
				return true;
			/* U+2060..U+2064: 0xE2 0x81 0xA0..0xA4 */
...
recopy:
	...
	/* Reject paths containing zero-width Unicode characters */
	if (path_has_zero_width_chars(kname, len)) {
		pr_warn_ratelimited(
			"VFS: rejected path with zero-width characters from %s[%d]\n",
