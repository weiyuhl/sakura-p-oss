...
static struct policydb *get_policydb(void)
{
    ...
    db = &policy->policydb;
#else
    /* On older kernels, access policydb through security server */
...
#else
/* On 3.18/4.x kernels, avc_ss_reset takes only a seqno */
extern int avc_ss_reset(u32 seqno);
#endif
...
#else
    /* On old kernels: no selinux_state, no selinux_avc */
    avc_ss_reset(0);
    selnl_notify_policyload(0);
    selinux_status_update_policyload(0);
#endif
