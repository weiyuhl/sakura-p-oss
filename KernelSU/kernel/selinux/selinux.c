...
/* selinux_cred() compat: on old kernels, cred->security is used directly */
...
/* selinux_inode() compat: on old kernels, inode->i_security is used directly */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0)
...
#else
    /* On older kernels, selinux_enforcing is a global int */
