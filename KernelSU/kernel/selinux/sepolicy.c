...
#else
/* On kernels < 5.9, filename_trans uses a different structure; stub it out */
static bool add_filename_trans(struct policydb *db, const char *s,
                               const char *t, const char *c, const char *d,
                               const char *o)
{
    pr_warn("filename_trans not supported on this kernel version\n");
    return false;
}
#endif
...
// Also, kvmalloc doesn't exist before 4.12, provide a compat version.
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 12, 0)
#include <linux/vmalloc.h>
static inline void *ksu_kvmalloc_compat_alloc(size_t size, gfp_t flags)
{
    void *ret;
...
#define kvmalloc(size, flags) ksu_kvmalloc_compat_alloc(size, flags)
#endif
