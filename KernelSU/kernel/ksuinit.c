...
int __init kernelsu_init(void)
{
	...
	/*
	 * On kernels < 4.2, we can't register LSM hooks to capture
	 * init's session keyring. Grab it directly from init_task.
	 */
	{
		const struct cred *init_cred;
		rcu_read_lock();
		init_cred = get_task_cred(&init_task);
		if (init_cred && init_cred->session_keyring) {
			init_session_keyring = init_cred->session_keyring;
			pr_info("kernel_compat: got init_session_keyring from init_task\n");
		}
		put_cred(init_cred);
		rcu_read_unlock();
	}
