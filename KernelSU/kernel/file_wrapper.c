...
/*
 * file_wrapper is a complex feature that requires many APIs not available
 * in kernel 3.18 (alloc_file_pseudo, security_inode_init_security_anon,
 * replace_fops, fops_get/fops_put, etc.). On old kernels, we provide
...
{
    pr_warn("file_wrapper: not supported on this kernel version\n");
    return -ENOSYS;
}

void ksu_file_wrapper_init(void)
{
    pr_info("file_wrapper: disabled on old kernel\n");
}

#else /* KERNEL_VERSION >= 4.14 */
