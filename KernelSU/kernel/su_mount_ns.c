...
#endif
#else
    /* On old kernels, call sys_setns directly */
...
#else
    /* path_mount not available, use do_mount */
    extern long do_mount(const char *, const char __user *, const char *, unsigned long, void *);
    /* On old kernels, we skip this step as do_mount requires userspace path */
    pm_ret = 0;
    pr_info("path_mount not available, skip make root private\n");
#endif
    path_put(&root_path);
