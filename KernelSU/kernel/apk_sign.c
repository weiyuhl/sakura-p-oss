...
/*
 * kernel_read() compat: on kernels < 4.14, the signature is:
 *   int kernel_read(struct file *, loff_t, char *, unsigned long);
 * On 4.14+:
 *   ssize_t kernel_read(struct file *, void *, size_t, loff_t *);
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0)
static inline int ksu_kernel_read(struct file *fp, void *buf, size_t count, loff_t *pos)
{
    int ret = kernel_read(fp, *pos, (char *)buf, count);
    if (ret > 0)
        *pos += ret;
    return ret;
}
